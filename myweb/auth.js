            ...
            if (desktop) {
                desktop.innerHTML = `
                    <div class="flex items-center gap-x-2 text-sm">
                        <div class="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-2xl border text-sm">${short}</div>
                        <button onclick="localStorage.removeItem('roadRulesUser'); location.reload()" class="px-3 py-1.5 text-xs border rounded-2xl">Sohoka</button>
                    </div>`;
            }
            if (mobile) {
                mobile.innerHTML = `<div class="text-center text-sm py-2">${short} • <span onclick="localStorage.removeItem('roadRulesUser');location.reload()" class="text-red-600 cursor-pointer">Sohoka</span></div>`;
            }
        };
